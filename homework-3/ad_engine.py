...
class AdEngine:
    ...
    def __init__(self, data_file, structure, dec_vars, util_map):
        ...
        state_names = ['P', 'A', 'G', 'I', 'T', 'F', 'H', 'S', 'Ad2', 'Ad1']

        _x = np.genfromtxt(data_file, delimiter=',')

        model = BayesianNetwork.from_structure(X=_x, structure=structure, state_names=state_names)
        print("hey")
...
class AdEngineTests(unittest.TestCase):
    ...
    # test_defendotron_ad_engine_t2 is the best there is
